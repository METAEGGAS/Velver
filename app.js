        ...
        return `<div class="tradeCard" data-idx="${i}">
            ...
            ${tr.closed ? `<div class="trDetails" id="trD${i}">
                <div class="trDRow">
                    ...
                    <span class="value">${tr.openPrice.toFixed(chart.d.digits)}</span>
                </div>
                <div class="trDRow">
                    <span class="label">سعر الإغلاق:</span>
                    <span class="value">${(tr.closePrice || 0).toFixed(chart.d.digits)}</span>
                </div>
                <div class="trDRow">
                    <span class="label">وقت الفتح:</span>
                    <span class="value">${fDT(tr.openTime)}</span>
                </div>
                <div class="trDRow">
                    <span class="label">وقت الإغلاق:</span>
                    <span class="value">${fDT(tr.closeTime || 0)}</span>
                </div>
                <div class="trDRow">
                    <span class="label">النتيجة:</span>
                    <span class="value ${pfC}">${pf > 0 ? "ربح" : pf < 0 ? "خسارة" : "تعادل"}</span>
                </div>
                <div class="trDRow">
                    <span class="label">المبلغ:</span>
                    <span class="value ${pfC}">$${pf.toFixed(2)}</span>
                </div>
...
class Chart {
    ...
    subLV() {
        ...
        this.liveUnsub = oS(D(db, "live", k), async s => {
            ...
            if (dt.mID) {
                ...
                } else if (!isMgr && (cM === sID || nw - lT > 15000)) {
                    isMgr = true;
                    sID = sessionStorage.getItem("sessID") || uid();
                    sessionStorage.setItem("sessID", sID);
                    await sD(D(db, "live", k), { mID: sID, t: Date.now() }, { merge: true });
    ...
    gridSteps() {
        ...
        let sT = nS(tS / 7);
        const al = [5000, 10000, 15000, 30000, 60000, 120000, 300000, 600000, 900000, 1800000, 3600000, 7200000];
    ...
    updateTimeLabels() {
        ...
        for (let i = s; i <= e; i++) {
            ...
            lb.style.left = x + "px";
            lb.textContent = (stepT < 60000 ? `${hh}:${mm}:${ss}` : `${hh}:${mm}`);
            tl.appendChild(lb);
        }
    }
    ...
    updateCandleTimer() {
        const nw = Date.now();
        const el = nw - this.t0;
        const lf = Math.max(0, Math.ceil((this.tf - el) / 1000));
...
document.querySelectorAll(".accSwitchBtn").forEach(bn => bn.onclick = () => {
    ...
    document.querySelectorAll(".accSwitchBtn").forEach(x => 
        x.classList.toggle("active", x === bn)
    );
    document.querySelectorAll(".accItem").forEach(x => 
        x.classList.toggle("active", x.dataset.type === aA)
    );
    upB();
    upAI();
});
