injectPage("cloud", {
  h: `
    <style>
      #cloudQuotesPage, #cloudQuotesPage * {
      ...
      #cloudQuotesPage {
        ...
        min-height: 100vh;
        overflow-x: hidden;
        font-family: "Noto Sans Arabic", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif;
        color: #fff;
        direction: ltr;
      }
      #cloudQuotesPage .app {
      ...
      #cloudQuotesPage .app.on { display: block; }
      #cloudQuotesPage .tabs {
        display: flex;
        align-items: center;
        gap: 28px;
        padding: 10px 4px 0;
      ...
      #cloudQuotesPage .tab.active { color: #fff; }
      #cloudQuotesPage .tab.active::after {
        content: "";
        position: absolute;
        bottom: 0;
        left: 0;
      ...
      #cloudQuotesPage .head > div:nth-child(2),
      #cloudQuotesPage .head > div:nth-child(3) {
        text-align: center;
      }
      #cloudQuotesPage .row {
        display: grid;
      ...
      #cloudQuotesPage .row:first-of-type { border-top: none; }
      #cloudQuotesPage .n .s {
        font-size: 26px;
        font-weight: 700;
        color: #fff;
      ...
      #cloudQuotesPage .p.up { color: #22c55e; }
      #cloudQuotesPage .p.dn { color: #e91e8c; }
      #cloudQuotesPage .c {
        justify-self: center;
        padding: 9px 22px;
        border-radius: 22px;
      ...
      #cloudQuotesPage .c.d { background: #c1196b; }
      #cloudQuotesPage .c.u { background: #22c55e; }
    </style>

    <div id="cloudQuotesPage" dir="rtl">
      <div class="app on" id="cloud_forex">
        <div class="tabs">
          <div class="tab active" data-target="cloud_forex">Forex</div>
          <div class="tab" data-target="cloud_metals">Precious Metals</div>
        ...
        <div class="card">
          ...
          <div id="cloud_fxL"></div>
        </div>
      </div>

      <div class="app" id="cloud_metals">
        <div class="tabs">
          <div class="tab" data-target="cloud_forex">Forex</div>
          <div class="tab active" data-target="cloud_metals">Precious Metals</div>
          <div class="search">🔍</div>
        </div>
        <div class="card">
          <div class="head">
...
(() => {
  const root = document.getElementById("cloudQuotesPage");
  if (!root) return;
  ...
  function upd(arr) {
    arr.forEach((d) => {
      const [s, p, c, , dc] = d;
      const id = rid(s);
      const pe = root.querySelector(`#p_${id}`);
      const ce = root.querySelector(`#c_${id}`);
      if (!pe || !ce) return;
  ...
  build(fx, root.querySelector("#cloud_fxL"));
  build(mt, root.querySelector("#cloud_mtL"));
