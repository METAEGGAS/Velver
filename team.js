...
m:async function(){
  try{
    ...
    async function genCode(uid){for(let i=0;i<8;i++){let c=String(Math.floor(100000+Math.random()*900000));let q=await getDocs(query(collection(db,"users"),where("inviteCode","==",c),limit(1)));if(q.empty){await setDoc(doc(db,"users",uid),{inviteCode:c},{merge:!0});return c}}let c=uid.slice(-6).toUpperCase();await setDoc(doc(db,"users",uid),{inviteCode:c},{merge:!0});return c}
