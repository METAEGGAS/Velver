...
m:async function(){
  try{
    ...
    const{getFirestore,doc,getDoc,setDoc,serverTimestamp,collection,getDocs,query,where,limit}=await import("https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js");
    ...
    async function genCode(uid){
      for(let i=0;i<10;i++){
        ...
        try{let q=await getDocs(query(collection(db,"users"),where("inviteCode","==",c),limit(1)));if(q.empty){await setDoc(doc(db,"users",uid),{inviteCode:c,inviteCodeCreatedAt:serverTimestamp()},{merge:!0});return c}}catch(e){}
      }
      let c=uid.slice(-6).toUpperCase();await setDoc(doc(db,"users",uid),{inviteCode:c,inviteCodeCreatedAt:serverTimestamp()},{merge:!0});return c
    }
    async function ensureCode(uid){
      try{let r=await getDoc(doc(db,"users",uid));if(r.exists()&&r.data().inviteCode)return r.data().inviteCode}catch(e){}
      return await genCode(uid)
    }
    function render(code,refs,tot){
      ...
      $("#tmINC").textContent=tot.toFixed(2);
    ...
    onAuthStateChanged(auth,async u=>{
      if(!u){$("#tmCODE").textContent="سجّل دخول";$("#tmLNK").textContent="—";return}
      let code=await ensureCode(u.uid);
      let refs=[],tot=0;
      try{
        let qs=await getDocs(query(collection(db,"users"),where("inviterUid","==",u.uid)));
        qs.forEach(x=>{let v=x.data();refs.push({uid:x.id,email:v.email,phone:v.phone,avatar:v.avatar,deposit:v.deposit||0,createdAt:v.createdAt});tot+=Number(v.deposit||0)});
